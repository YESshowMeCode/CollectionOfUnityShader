...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    // 烘焙GI的实现：首先从烘焙的光照贴图中取得对应的像素值，其次根据编码对像素进行译码。
    // 如果定义了directional lightmap混合，会进行和烘焙光照贴图类似的过程并进行混合计算。否则的话，直接使用烘焙GI的颜色进行计算
        ...
        half3 bakedColor = DecodeLightmap(bakedColorTex);
            ...
            o_gi.indirect.diffuse += DecodeDirectionalLightmap (bakedColor, bakedDirTex, normalWorld);
                ...
                ResetUnityLight(o_gi.light); //将UnityGI结构体初始化
...
inline UnityGI UnityGlobalIllumination (UnityGIInput data, half occlusion, half smoothness, half3 normalWorld, bool reflections)
{
    if(reflections)
    {
        // 有反射：调用UnityGI_Base函数，得到返回值o_gi 然后调用UnityGI_IndirectSpecular修改o_gi.indirect.specular的值
